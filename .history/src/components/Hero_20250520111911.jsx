...
const Hero = ({ slideOut, onEnter }) => {
	...
	useEffect(() => {
		if (!slideOut) {
			// Hero just appeared or should slide in
			setSlideIn(true);
		} else {
	...
	return (
		<div
			className={`hero-container transition-transform duration-1000 ease-in-out ${
				slideIn ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"
			}`}
		>
