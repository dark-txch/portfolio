...
export default function Home() {
	...
	const handleClick = () => {
		if (!slideOut) {
			...
			setTimeout(() => {
				setShowMain(true);
			}, 1000); // Matches hero section duration
		} else {
			setShowMain(false);
			setTimeout(() => {
				setSlideOut(false);
			}, 1000); // Wait for main to slide down
		}
	};
		...
		<>
			...
			<div
				className={`relative w-screen min-h-screen ${
					slideOut ? "overflow-auto" : "overflow-hidden"
				}`}
			>
				{/* hero header */}
				<header
				...
				<main
					className={`w-screen bg-[#0C0950] transform transition-all duration-1000 ease-in-out ${
						showMain ? "translate-y-0" : "translate-y-full"
					}`}
				>
					<header className="w-screen h-auto bg-[#0C0950] pt-[5rem]">
						<nav className="relative text-[1rem] w-screen shadow flex justify-between px-[1rem] py-[1.5rem] uppercase lg:flex lg:items-center lg:justify-between lg:px-[3rem] border border-red-500">
						...
						{/* Body of the header */}
						<h1 className="font-unna text-cyan-200 px-[1rem] leading-[3rem] mt-[5rem] text-[2.5rem] mb-[0.8rem] lg:w-[65%] lg:mx-auto lg:text-[3rem]">
							Hello, I'm Ebenezer Tiewei &#x1F44B;
						</h1>
						<p className="font-unna text-[#EAE4D5] text-[1.8rem] leading-[3rem] px-[1rem] mb-[2rem] lg:w-[65%] lg:text-[2.2rem] lg:mx-auto">
