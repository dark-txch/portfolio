		...
		<div className="w-screen h-auto bg-[#09063a] text-[1.4rem] pb-[5rem] lg:text-[1.6rem] lg:px-[3rem]">
			...
			<div className="text-[#EAE4D5] px-4 mt-10">
				<button
					...
					className="uppercase border flex gap-3 items-center border-cyan-500 text-cyan-200 text-[1rem] px-6 py-2 rounded-full hover:bg-cyan-500 hover:text-[#0C0950] transition-all duration-300"
				>
					...
					<div className="mt-6 space-y-6">
						{skillCategories.map((category) => (
							<div key={category.title}>
								<h4 className="text-[1.4rem] font-bold mb-3">
									{category.title}
								</h4>
								<ul className="space-y-2 text-[1.2rem] flex items-center border-[0.2]">
									{category.skills.map((skill) => (
										<li key={skill} className="flex items-center gap-2">
											<GoDotFill className="flex items-center text-cyan-400" />
											<span>{skill}</span>
										</li>
									))}
								</ul>
