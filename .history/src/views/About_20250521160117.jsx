...
export default function About() {
	...
	return (
		<div className="w-screen h-auto bg-[#09063a] text-[1.3rem] pb-[5rem] lg:px-[3rem]">
			<div className="flex flex-col justify-center lg:flex-row">
				<div className="border-[0.2rem] border-cyan-100 lg:w-[70%] order-1">
					...
					<p className="font-thin text-[#EAE4D5] te leading-[2.5rem] px-[1rem] mb-[2rem] lg:text-[2.2rem]">
			...
			<div className="text-[#EAE4D5] px-4 mt-10">
				<button
					onClick={() => setShowSkills(!showSkills)}
					className="uppercase border flex gap-3 items-center border-cyan-500 text-cyan-200 px-6 py-2 rounded-full hover:bg-cyan-500 hover:text-[#0C0950] transition-all duration-300"
				>
					...
					<div className="mt-6 space-y-6">
						{skillCategories.map((category) => (
							<div key={category.title}>
								<h4 className="text-[1.8rem] lg:te font-light mb-3">
									{category.title}
								</h4>
								<ul className="space-y-2">
									{category.skills.map((skill) => (
										<li
											key={skill}
											className="flex items-center gap-2 te"
										>
