...
export default function About() {
	const skillSet = () => {
		...
		const skillsCategory = [
			...
			{ title: "Tools", skills: ["Git", "GitHub", "Figma"] },
		];
	};
		...
		<div className="w-screen h-auto bg-[#09063a] lg:px-[3rem]">
			...
			<div className="font-unna font-bold text-cyan-200 px-[1rem] leading-[3rem] mt-[5rem] text-[2.5rem] mb-[1.5rem] lg:text-[3rem]">
				<button
					...
					className="border-[0.2rem] border-cyan-100 flex gap-2 items-center text-[1.8rem] cursor-pointer relative"
				>
					<FaHandPointRight size={25} />
					skills
				</button>
				{showSkills && (
					<div className="absolute bg-[#09063a] border-[0.2rem] border-cyan-100 w-[90%] lg:w-[50%] h-auto flex flex-col gap-5 p-5">
						{skillSet().map((category) => (
							<div key={category.title}>
								<h4 className="text-[#EAE4D5] text-[1.8rem] leading-[2.5rem] px-[1rem] mb-[2rem] lg:text-[2.2rem]">
									{category.title}
								</h4>
								<ul className="flex flex-col gap-3">
									{category.skills.map((skill) => (
										<li
											key={skill}
											className="flex items-center text-[#EAE4D5] text-[1.8rem] leading-[2.5rem]"
										>
											<GoDotFill />
											{skill}
										</li>
